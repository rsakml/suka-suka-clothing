...
const IndexCelana = () => {
  const [celana, setCelanas] = React.useState([]);

  React.useEffect(() => {
    const grs = onSnapshot(collection(db, "celana"), (snapshot) => {
      setCelanas(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    });
    return grs;
  }, []);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'celana', id));
      MySwal.fire({
        icon: 'success',
