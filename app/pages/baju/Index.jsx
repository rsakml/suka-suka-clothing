...
const IndexBaju = () => {
  const [baju, setBajus] = React.useState([]);

  React.useEffect(() => {
    const grs = onSnapshot(collection(db, "baju"), (snapshot) => {
      setBajus(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    });
    return grs;
  }, []);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'baju', id));
      MySwal.fire({
        icon: 'success',
