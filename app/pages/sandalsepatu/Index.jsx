...
const IndexSandalSepatu = () => {
  const [sandalsepatu, setSandalSepatus] = React.useState([]);

  React.useEffect(() => {
    const grs = onSnapshot(collection(db, "sandalsepatu"), (snapshot) => {
      setSandalSepatus(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    });
    return grs;
  }, []);

  console.log(sandalsepatu);

  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'sandalsepatu', id));
      MySwal.fire({
        icon: 'success',
...
export default IndexSandalSepatu;
